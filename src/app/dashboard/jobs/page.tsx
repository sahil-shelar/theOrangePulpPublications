...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { icon: React.ReactNode; cls: string }> = {
    ...
    cancelled: { icon: <Ban size={13} />,                                cls: 'text-foreground/40 bg-muted' },
  }
          ...
          <tbody>
            {(!jobs || jobs.length === 0) ? (
              ...
              <tr key={job.id} className="border-t-[3px] border-foreground hover:bg-muted transition-colors">
                ...
                <td className="p-3.5 hidden md:table-cell text-xs font-bold text-foreground/60">{job.attempts ?? 0}</td>
                <td className="p-3.5 hidden md:table-cell text-xs font-bold text-foreground/60">
