  ...
  return (
    <Link href={href} className="group block brutal-card p-0 overflow-hidden hover:-translate-y-1 transition-transform">
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        <div className={`${accentColor} on-pastel p-8 md:p-12 flex flex-col justify-center gap-4`}>
          <div className="flex items-center gap-2">
            ...
            {post.categories?.name && (
              <span className="dim text-label font-black uppercase tracking-widest">{post.categories.name}</span>
            )}
          </div>
          ...
          {post.excerpt && (
            <p className="dim text-base font-medium leading-relaxed line-clamp-3">{post.excerpt}</p>
          )}
          <div className="dim flex items-center justify-between text-label font-black uppercase tracking-widest pt-4 border-t-[2px] border-foreground/20">
